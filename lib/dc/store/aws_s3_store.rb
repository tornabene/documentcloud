module DC
  module Store
    ...
    module AwsS3Store
      ...
      module ClassMethods
        def asset_root
          Rails.env.production? ? "http://s3.documentcloud.org:8080" : "http://s3.documentcloud.org:8080/#{BUCKET_NAME}"
        end
        def web_root
          Thread.current[:ssl] ? "https://s3.documentcloud.org/#{BUCKET_NAME}" : asset_root
        end
      end
